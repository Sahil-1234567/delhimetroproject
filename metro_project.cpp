...
int main(){
  ...
  /*if (response == "1") {
      ...
      string src_station, dt_station;

      cout << "Enter starting station: ";   
      cin.ignore(); // Ignore previous newline character
      getline(cin, src_station);

      cout << endl << "Enter ending station: ";   
      getline(cin, dt_station);

      // Check if the entered stations exist in the metro map
      if (metro.find(src_station) == metro.end() || metro.find(dt_station) == metro.end()) {
          cout << "Invalid station name. Please enter valid station names." << endl;
          return 1; // Exit with error code
      }

      int src = metro[src_station];
      int dt = metro[dt_station];

      cout << endl << "Starting Station is " << src_station << " and Ending Station is " << dt_station << endl;

      dijkstra(graph, src, dt, cnt);

      cout << "\n\nNumber of stations in between these two stations are: " << cnt.size() << endl;
  }
  // -----------------------------------------------------
  if(response == "1"){
    vector<int> cnt;
  ...
  if (response == "1") {
      ...
      // Convert station names to station indices
       src = metro[src_station];
      dt = metro[dt_station];

      cout << endl << "Starting Station is " << src_station << " and Ending Station is " << dt_station << endl;

      // Now, src and dt are properly defined and can be used in dijkstra function
      dijkstra(graph, src, dt, cnt);
